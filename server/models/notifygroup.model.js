...
const NotifyGroupSchema = new Schema({
  distinction: {
    type: String,
  ...
  collection: 'notifygroups',
  read: 'nearest'
});
NotifyGroupSchema.index({
  distinction: 'text',
  username: 'text',
  usertype: 'text',
  mobile: 'text'
...
NotifyGroupSchema.plugin(mongoosePaginate);

const NotifyGroup = mongoose.model('Notifygroup', NotifyGroupSchema);
...
notifygroupsPaginated = async (page, limit, newestFirst) => {
  ...
  let result = await NotifyGroup.paginate(query, options);
  return result;
}
